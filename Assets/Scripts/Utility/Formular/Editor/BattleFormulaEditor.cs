...
public class BattleFormulaEditor : UnityEditor.Editor
{
    ...
    private float ElementHeightCallback(int index)
    {
        ...
        CalculationTargetType calculationTargetType =
            (CalculationTargetType)relativeObject.FindProperty("calculationTargetType").enumValueIndex;
    ...
    public override void OnInspectorGUI()
    {
        ...
        BattleFormulaInfo resultInfo = (BattleFormulaInfo)formulaInfoListProp
            .GetArrayElementAtIndex(formulaInfoListProp.arraySize - 1).objectReferenceValue;
...

        {
            EditorGUILayout.SelectableLabel(
                $"<color=lime><b>{(string.IsNullOrEmpty(formulaNameProp.stringValue) ? "It" : formulaNameProp.stringValue)}</b></color>'s Formula Finalized Value -> <color=lime>{resultInfo.GetEditorCalculatedValue()}</color>",
    ...
    private void DrawFormulaInfoListElement(Rect rect, int index, bool isActive, bool isFocused)
    {
        ...
        if (element.objectReferenceValue is null)
        {
            element.objectReferenceValue =
                EditorGUI.ObjectField(rect, GUIContent.none, null, typeof(BattleFormulaInfo), false);
            return;
        }
        ...
        SerializedProperty calculationTypeProp = relativeObject.FindProperty("formulaCalculationType");
        ...
        else
        {
            ...
            EditorGUI.SelectableLabel(rect,
                "<color=lime>On runtime, by searching statusName</color>, baseValue is set. ",
                new GUIStyle(GUI.skin.box) { richText = true, fontSize = 12 });
        ...
        switch (calculationTargetType)
        {
            case CalculationTargetType.UseModifyValue:
                ...
                break;
            case CalculationTargetType.UseFormulaInfoValue:
                SerializedProperty modifyBattleFormulaInfoProp =
                    relativeObject.FindProperty("modifyBattleFormulaInfo");
                modifyBattleFormulaInfoProp.objectReferenceValue = EditorGUI.ObjectField(rect,
                    modifyBattleFormulaInfoProp.objectReferenceValue, typeof(BattleFormulaInfo), false);
                if (modifyBattleFormulaInfoProp.objectReferenceValue != null)
                {
                    BattleFormulaInfo subFormulaInfo =
                        (BattleFormulaInfo)modifyBattleFormulaInfoProp.objectReferenceValue;
        ...
        EditorGUI.LabelField(labelPos,
            $"{(useClmapProp.boolValue ? "Clamp Mode <color=lime>On</color>" : "Clamp Mode <color=yellow>Off</color>")}",
            new GUIStyle(GUI.skin.label) { richText = true });
        rect.x += labelWidth;
        ...
        if (useClmapProp.boolValue)
        {
            rect.x += rect.width;
            rect.width = propertyWidth;
            clampRangeVector2Prop.vector2Value =
                EditorGUI.Vector2Field(rect, GUIContent.none, clampRangeVector2Prop.vector2Value);
        ...
        if ((FormulaCalculationType)calculationTypeProp.enumValueIndex is FormulaCalculationType.Logarithmic
                ...
                || float.IsInfinity(preCalculatedResult)))
        {
            ...
            EditorGUI.LabelField(rect, "<color=yellow><b>Invalid input value</b></color>. Please, check.",
    ...
    private string GetDescription(SerializedProperty element)
    {
        ...
        switch (formulaCalculationType)
        {
            case FormulaCalculationType.None:
                break;
            case FormulaCalculationType.Additive:
                formulaDescription += $"{formulaInfo.baseValue} +";
                break;
            case FormulaCalculationType.Multiply:
                formulaDescription += $"{formulaInfo.baseValue} ×";
                break;
            case FormulaCalculationType.Logarithmic:
